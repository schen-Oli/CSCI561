...
class Board:
    ...
    def is_valid_move(self, row, col, player):
        ...
        # in board
        inRange = row >= 0 and row < BOARD_SIZE and col >= 0 and col < BOARD_SIZE and self.state[row][col] == 0

        # check liberty
        liberty = self.checkLiberty(self, row, col, "up") 
        + self.checkLiberty(self, row, col, "down")
        + self.checkLiberty(self, row, col, "left")
        + self.checkLiberty(self, row, col, "right")
        
        
            
        return liberty

    def checkLiberty(self, player, row, col, direction):
        """
        ...
        if direction == "up" or "down":
            currRow = row + step
            while currRow >= 0 and currRow < BOARD_SIZE:
                if self.state[currRow][col] == 0:
                    return 1
                elif self.state[currRow][col] != player:
                    return 0
                else:
                    currRow += step
        ...
        if direction == "left" or "right":
            currCol = col + step
            while currCol >= 0 and currCol < BOARD_SIZE:
                if self.state[row][currCol] == 0:
                    return 1
                elif self.state[row][currCol] != player:
                    return 0
                else:
                    currCol += step
